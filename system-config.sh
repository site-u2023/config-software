...
do
  ...
  case "${num}" in
    "y" ) sed -i -e "s/HOSTNAME='openwrt'/HOSTNAME=${input_str_SYSTEM_HOSTNAME}/g" /etc/device-config.sh; _func_ROOT_PASSWD ;;
    "n" ) _func_HOSTNAME ;;
    "e" ) exit ;;
  esac
done
...
do
  ...
  read -p " 本当に宜しいですか? [y/n or e]: " num
  case "${num}" in
    "y" ) sed -i -e "s/ROOT_PASSWD/${input_str_ROOT_PASSWD}/g" /etc/device-config.sh; _func_WIFI_SSID_A ;;
    "n" ) _func_ROOT_PASSWD ;;
    "e" ) exit ;;
  esac
done
...
do
  ...
  read -p " 本当に宜しいですか? [y/n or e]: " num
  case "${num}" in
    "y" ) sed -i -e "s/WIFI_SSID_A='SSID_A'/WIFI_SSID_A=${input_str_WIFI_SSID_A}/g" /etc/device-config.sh; _func_WIFI_PASSWD_A ;;
    "n" ) _func_WIFI_SSID_A ;;
    "e" ) exit ;;
  esac
done
...
do
  ...
  read -p " 本当に宜しいですか? [y/n or e]: " num
  case "${num}" in
    "y" ) sed -i -e "s/wifi_PASSWORD_A='password'/wifi_PASSWORD_A=${input_str_WIFI_PASSWD_A}/g" /etc/device-config.sh; _func_WIFI_SSID_B ;;
...
do
  ...
  read -p " 本当に宜しいですか? [y/n or e]: " num
  case "${num}" in
    "y" ) sed -i -e "s/WIFI_SSID_B='SSID_B'/WIFI_SSID_B=${input_str_WIFI_SSID_B}/g" /etc/device-config.sh; _func_WIFI_PASSWD_B ;;
    "n" ) _func_WIFI_SSID_B ;;
    "e" ) exit ;;
  esac
done
...
do
  ...
  read -p " 本当に宜しいですか? [y/n or e]: " num
  case "${num}" in
    "y" ) sed -i -e "s/wifi_PASSWORD_B='password'/wifi_PASSWORD_B=${input_str_WIFI_PASSWD_B}/g" /etc/device-config.sh;
