...
while :
do
  echo -e " \033[1;32m推奨パッケージインストール（自動or選択）のスクリプトをダウンロードします\033[0;39m"
  read -p " 宜しいですか? [y/n]: " num
  case "${num}" in
...
while :
do
  echo -e " \033[1;37mJapanese notation\033[0;39m"
  echo -e " \033[7;33m推奨設定順序: システム > インターネット > パッケージ\033[0;39m"  
  echo -e " \033[1;37mopenwrt-config --------------------------------------\033[0;39m"
  ...
  echo -e " \033[1;36m[e]\033[0;39m": 他：ボタン（スイッチ） / IPERF3 / WiFi位置情報サービス停止設定
