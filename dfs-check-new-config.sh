...
if [ -n "$1" ]; then
	...
	do
		ORIGIN=`cat /etc/init.d/dfs_check_new | awk '{print substr($0,index($0,"=") )}'`
		ORIGIN1=`echo ${ORIGINA}  | grep -o "[0-9]*" | head -1`
		...
		case "${num}" in
		"y" ) logger "DFS Check NEW: Interval change"
